...
docker build -t $REGISTRY$NAME_IAC_SOCKETCAN_DBW --target=socketcan_dbw_bridge --platform=linux/amd64 -f Dockerfile ..
echo "---------- build ros2_foxglove_bridge ----------"
docker build -t $REGISTRY$NAME_FOXGLOVE --target=ros2_foxglove_bridge --platform=linux/amd64 -f Dockerfile ..
