...
def parameterizeOrchestrator(maPort, number_of_clients):
    ...
    maPort.Write(paramStaticCosim_signal_path, MyValueFactory.CreateFloatValue(0.0))
...
if __name__ == "__main__":
    ...
    MAPort = None
    valid_commands = ["start", "stop", "reset", "status", "healthcheck", "noclif", "loadonly", "manualflag", "trackflag", "vehicleflag", "deactivatepush2pass", "synchrace", "orchparam", "manualrace", "raceparticipant", "velocity", "raceparam"]
    ...
    if command == "healthcheck" and not is_orchestrator:
            ...
            if maneuverState.Value != maneuverStateLastValue:
                with open("/home/dspace/scripts/maneuverstatus.sh", "w") as fileStatus:
                    if maneuverState.Value == 3:
                        ...
                        print("Maneuver state unhealthy:", maneuverState.Value)
                        fileStatus.write("exit 1")
                maneuverStateLastValue = maneuverState.Value
            time.sleep(0.5)

    elif command == "healthcheck" and is_orchestrator:
        # Add check for orchestrator status
        print("Orchestrator state changed to healthy")
        with open("/home/dspace/scripts/maneuverstatus.sh", "w") as fileStatus:
            fileStatus.write("exit 0")
        while True:
            time.sleep(10)
    ...
    try:
        ...
        for command in command_list:
            ...
            #         fileStatus.write("exit 1")
            ...
            elif command == "manualrace":
                MAPort.Write('ASM_Traffic/Model Root/VesiInterface/Sw_Activate_CLIF[0|1]/Value', MyValueFactory.CreateFloatValue(1.0))
                MAPort.Write('ASM_Traffic/Model Root/VesiInterface/Sw_Manual_VESI_Overwrite[0|1]/Value', MyValueFactory.CreateFloatValue(1.0))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/Sw_RaceControl[0Intern|1Extern|2Orchestrator]/Value', MyValueFactory.CreateFloatValue(0.0))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_sys_state/Value', MyValueFactory.CreateUintValue(9))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_track_flag/Value', MyValueFactory.CreateUintValue(1))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_veh_flag/Value', MyValueFactory.CreateUintValue(0))
                MAPort.Write('ASM_Traffic/Model Root/VesiInterface/VESIResultData_Manual/vehicle_inputs/Const_enable_brake_cmd/Value', MyValueFactory.CreateUintValue(1))
                MAPort.Write('ASM_Traffic/Model Root/VesiInterface/VESIResultData_Manual/vehicle_inputs/Const_enable_gear_cmd/Value', MyValueFactory.CreateUintValue(1))
                MAPort.Write('ASM_Traffic/Model Root/VesiInterface/VESIResultData_Manual/vehicle_inputs/Const_enable_steering_cmd/Value', MyValueFactory.CreateUintValue(1))
                MAPort.Write('ASM_Traffic/Model Root/VesiInterface/VESIResultData_Manual/vehicle_inputs/Const_enable_throttle_cmd/Value', MyValueFactory.CreateUintValue(1))
                print("Configure manual race inputs.")

            elif command == "raceparticipant":
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/Sw_RaceControl[0Intern|1Extern|2Orchestrator]/Value', MyValueFactory.CreateFloatValue(0.0))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_sys_state/Value', MyValueFactory.CreateUintValue(9))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_track_flag/Value', MyValueFactory.CreateUintValue(1))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_veh_flag/Value', MyValueFactory.CreateUintValue(0))
                velocity_value = int(os.environ.get("TARGET_SPEED", 185))
                MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_round_target_speed/Value', MyValueFactory.CreateUintValue(velocity_value))
                print("Configure race participant.")
                ...
                if len(command_parts) > 1:
                    try:
                        velocity_value = int(command_parts[1])
                        
                        MAPort.Write('ASM_Traffic/Model Root/RaceControl/race_control/Const_round_target_speed/Value', MyValueFactory.CreateUintValue(velocity_value))
                        print(f"Target speed set to: {velocity_value}")
                    except ValueError:
                        print("The part after 'velocity_' is not valid.")
                else:
                    print("The velocity command does not contain an underscore followed by a value.")
